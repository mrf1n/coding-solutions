package com.github.mrf1n.catcoder.T4344

import com.github.mrf1n.catcoder.CccSolutionWrapper
import com.github.mrf1n.catcoder.CccSolver
...
class T4344_Level5 : CccSolver {
    ...
    override fun solve() {
        solutionWrapper.runLevel { fileName, input, output ->
            ...
            for (i in 0 until numberOfMinutes) {
                ...
                minutes.add(Minute(i + 1, maxPower, maxConcurrentTasks, price))
            }
    ...
    private fun calculateTask(minutes: List<Minute>, task: Task): Long {
        while (task.remainingPower > 0) {
            val minute = minutes.first { minute ->
                minute.remainingPower > 0 && task.startRange <= minute.id && task.endRange >= minute.id &&
                        (minute.tasks.count() < 2 || minute.tasks.contains(task.id))
            }
